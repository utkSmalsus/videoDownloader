...
interface SidebarProps {
  ...
  onNavigate?: () => void;
  /** Namespaces the sliding-indicator layoutId. The desktop rail and the mobile drawer
   *  can be mounted at the same time; without this they'd share one layout animation
   *  and the indicator would fly between the two panels. */
  instanceId?: string;
}

export function Sidebar({ collapsed = false, onToggleCollapse, onNavigate, instanceId = "desktop" }: SidebarProps) {
          ...
          return (
            <li key={href}>
              ...
              >
                {active && (
                  <>
                    {/* Both the tint and the rail share one layoutId pair, so switching
                        platforms slides the highlight down the nav instead of blinking. */}
                    <motion.span
                      layoutId={`${instanceId}-nav-active`}
                      transition={{ type: "spring", stiffness: 400, damping: 34 }}
                      className="absolute inset-0 rounded-[var(--radius-sm)] bg-platform-soft"
                      aria-hidden
                    />
                    <motion.span
                ...
                <Icon className="relative size-[18px] shrink-0 transition-transform duration-200 group-hover:scale-110" aria-hidden />
